// entrypoint.js
// Rewritten from entrypoint.sh using Octokit for GitHub API interactions
...
async function main() {
  ...
  if (!PR_NUMBER || !COMMENT_BODY) {
    console.error('Could not determine PR number or comment body from event payload.');
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(`Failed to fetch file '${FILE_TO_REVERT}' from base branch:`, err.message);
    process.exit(1);
  }

  if (!baseFile.data.content || !baseFile.data.sha) {
    console.error('Could not retrieve file content or sha from base branch.');
    process.exit(1);
  }

  // Get file sha from head branch (needed for update)
  let headFileSha = undefined;
  try {
    ...
    if (err.status !== 404) {
      console.error(`Failed to fetch file from head branch:`, err.message);
      process.exit(1);
    }
  }
  ...
  try {
    ...
    console.error('Failed to update file in head branch:', err.message);
    process.exit(1);
  }
}
